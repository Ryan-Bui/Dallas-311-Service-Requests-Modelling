...
def ingest_pptx():
    ...
    filename = "311_Presentation_Final (2).pptx"
    ...
    try:
        ...
        for slide_num, slide in enumerate(prs.slides, 1):
            ...
            if slide_text:
                extracted_text.append(f"--- Slide {slide_num} ---\n" + "\n".join(slide_text))
                ...
                for offset, (chunk_text, embedding) in enumerate(zip(batch_chunks, embeddings)):
                    ...
                    session.run("""
                        ...
                        SET c.content = $content,
                            ...
                            c.source = $source,
                            c.type = $type
                    """, id=chunk_id, content=chunk_text, embedding=embedding, chunk_index=chunk_index, source=filename, type="Presentation")
                    ...
                    for d_name, d_id in dept_map.items():
                        if d_name.lower() in chunk_text.lower():
                            ...
                            break
        print(f"  Successfully ingested {filename} into Neo4j.")
    except Exception as e:
